...
//    dataSetStonesAndChips = [self loadObjectTrackerDataSet:@"test.xml"];
    dataSetTarmac = [self loadObjectTrackerDataSet:@"gubei3.xml"];
    if (dataSetTarmac == NULL) {
        NSLog(@"Failed to load datasets");
        return NO;
    }
    if (! [self activateDataSet:dataSetTarmac]) {
...
//            [menuVC setValue:extendedTrackingEnabled forMenuItem:@"Extended Tracking"];
            ...
            if (dataSetCurrent == dataSetTarmac) {
